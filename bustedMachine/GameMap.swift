...
let porchOfAShotgunshack = Node(location: "Porch of A Shotgun Shack", description: "You find yourself standing in front of a SHOTGUN SHACK. It looks very weathered. A heavy LOCK is attached to the door. Someone has spraypainted the words 'Monster Party' and a crude DRAWING of a googly-eyed monster across the front porch.", objects: [ShotgunShack.name: ShotgunShack, Drawing.name: Drawing, Lock.name: Lock])
let frontroom = Node(location: "Front Room", description: "You find yourself inside a shotgun shack. The front room is vividly painted orange and green. You see a Naugahyde SOFA, a wooden coffee TABLE, a plastic beach CHAIR, and a metal CABINET with a CRT monitor inside.", objects: [Table.name: Table, Chair.name: Chair, Cabinet.name: Cabinet, CRT.name: CRT, Sofa.name: Sofa])
let bedroom = Node(location: "Bedroom", description: "You find yourself inside a shotgun shack. This narrow room is dominated by a queen-size MATTRESS that lies flat on the floor. A tangle of twinkling FAIRYLIGHTS covers the ceiling. On the MATTRESS is an ASHTRAY and some torn NAPKINS.", objects: [Fairylights.name: Fairylights, Ashtray.name: Ashtray, Napkins.name: Napkins, Mattress.name: Mattress])
let makeshiftBedroom = Node(location: "Makeshift Bedroom", description: "You find yourself inside a shotgun shack. This is a hallway that is also a bedroom. Or a bedroom that is also a hallway. There is an empty bed frame and a chest of DRAWERS. Crumpled drywall is piled by the back wall.", objects: [Drawers.name: Drawers])
let kitchen = Node(location: "Kitchen", description: "You find yourself inside a shotgun shack. This kitchen is vividly painted green and orange. Someone has spraypainted 'Humans suck!' on the wall. There is a REFRIGERATOR, and many open CABINETS. There is a big HOLE in the floor by the SINK.", objects: [Sink.name: Sink, Refrigerator.name: Refrigerator, Cabinets.name: Cabinets, Hole.name: Hole])
...
let secondBasement = Node(location: "Second Basement", description: "You find yourself inside a shotgun shack. Down another set of creaky stairs is the second basement. It is as big as the house is long. The floor is spread with shag carpeting. Colorful GRAFFITI and stickers cover the beige TILES." , objects: [Tiles.name: Tiles, Graffiti.name: Graffiti], npcs: [Cookiepuss.name: Cookiepuss, Possum.name: Possum, FutureCreep.name: FutureCreep])
...
func generateSmoke() -> Graph {
    ...
    Cookiepuss.appearance = "\(Cookiepuss.name) is a Monster. \(Cookiepuss.pronouns[0].capitalized) has big, googly eyes, shaggy orange hair, droopy pink eyelids, and a crooked pink nose. \(Cookiepuss.pronouns[0].capitalized) is mostly covered in pink felt. \(Cookiepuss.pronouns[0].capitalized) wears a purple trucker cap, black high-top sneakers, a fanny pack, and a jock strap."
    Possum.appearance = "\(Possum.name) is a Monster. \(Possum.pronouns[0].capitalized) has big, googly eyes, a pointy snout, spiky green hair, and black bat-like ears. \(Possum.pronouns[0].capitalized) is covered in stained white felt. \(Cookiepuss.pronouns[0].capitalized) wears a septum piercing, fishnets, a fanny pack, leather hot pants, and black boots."
